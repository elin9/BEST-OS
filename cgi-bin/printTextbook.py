...
def main():
    ...
    if str(school)== "None" or str(school) == "base":
    	for row in c.execute('select * from bookposts order by datePosted desc'):
    	    print "<div class = \"post\" style = \"border: 1px solid #000000;\">Seller: %s | Title: %s | Author: %s | Edition: %s | ISBN: %s | Condition: %s | Other Notes: %s | Course Number: %s | Photo: <img style = \"width: 50px; height: 70px;\" src = \"%s\"> | Price: $%s | School: %s | Course: %s | Date Posted: %s GMT </div>" % row
    	    print("<br><br>")
    else:
